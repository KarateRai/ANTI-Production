...
public class ThickBOI_AI : AI
{
    ...
    private void ContructBehaviorTree()
    {
        ...
        //SetCastingNode castingRegenNode = new SetCastingNode(controller, true);
        RegenNode regenHealthNode = new RegenNode(controller, controller.Stats.MaxHealth); //Regen health
        Sequencer startRegenSequence = new Sequencer(new List<Node> { invertdmgTakenNode, /*castingRegenNode,*/ regenHealthNode }); //Sequence for health regen
        //SetCastingNode stopCastingRegenNode = new SetCastingNode(controller, false);
        Selector p2Selector = new Selector(new List<Node> {shieldLimiterNode, startRegenSequence, /*stopCastingRegenNode*/ }); //Phase 2 selector
