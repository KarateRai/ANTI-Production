...
public static class TargetsInRange
{
    ...
    public static List<UnitController> GetControllersInRange(float angle, float range, Transform transform, Collider ownCollider, LayerMask targetLayer)
    {
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            if (Vector3.Angle(transform.forward, directionToTarget) < angle / 2)
            {
                    ...
                    && targetsInViewRadius[i] != ownCollider)
                {
                    if (target.layer != ownCollider.gameObject.layer)
                    {
                        targetsInRange.Add(target.GetComponent<UnitController>());
                    }
                }
            }
            Debug.DrawLine(transform.position, targetTansform.position, Color.white);
